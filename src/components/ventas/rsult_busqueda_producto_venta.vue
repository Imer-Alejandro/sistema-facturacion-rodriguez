<script>
import productoVenta from './producto_ventas.vue';
...
export default {
    ...
    methods:{
        close_resultado_producto_venta() {
            emitter.emit('cerrar_busqueda_producto_venta')
        },
...
<template>
    <section  class="  w-full h-[100vh] bg-white fixed  z-40 ">
        <header class="w-full p-3 inline-flex border-b-[1.5px] border-b-[#DFDFDF]">
            <img  @click="close_resultado_producto_venta"  class="w-[35px] mr-[25px]  order-1 cursor-pointer" src="/src/assets/iconos/interfaz/regresar.png" >
        ...
        <section class="p-2 pt-[20px] bg-slate-100">
            ...
            <article class="p-2 mt-[20px]">
                <h3 class="  text-[1.2rem]  mb-[10px]">Productos encontrados: <span class="bg-[#647eff] p-1 rounded text-white">{{DatosProducto.length}}</span></h3>
                <div class="w-full h-[500px] overflow-hidden box-border overflow-y-scroll ">
                   
                 
                   <div v-for="productos in DatosProducto" :key="productos.id">
